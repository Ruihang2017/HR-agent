...
4. **Real unlawful-question filter (F5.3)** — prohibited categories as config (code decides, model interprets); blocks with reason + grounded compliant rewrite; applies to generated *and* manually added questions. ⚠️ Known Phase 1 latent defect to resolve here: `generate_kit` raises `ValidationError` (InterviewKitSchema `min_length=1`) if the filter blocks every question — decide rewrite-vs-drop semantics, then fix kit persistence for the all-blocked case (found in Task 11 review, dormant under the stub filter).
